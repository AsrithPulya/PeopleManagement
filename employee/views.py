...
class LeavePolicyCreateView(APIView):
    def post(self, request):
        ...
        if policy_serializer.is_valid():
            ...
            # Add the leave_policy ID to policy_type_data
            policy_type_data['leave_policy'] = policy.id
            
            # Serialize the LeavePolicyTypes data with the linked policy
            ...
            # Rollback policy creation if types data fails
            policy.delete()
...
class LeavePolicyUpdateView(APIView):
    def put(self, request, pk):
        ...
        # Get the updated data for the policy and policy type
        policy_data = request.data.get('policy', {})
        policy_type_data = request.data.get('policy_type', {})

        # Validate and update the LeavePolicy instance
...
class ApplyForLeaveView(APIView):
    def post(self, request):
        try:
            # Attempt to retrieve the associated Employee instance
            employee = request.user.employee  
            print("Employee instance:", employee)  # Debug print
        except Employee.DoesNotExist:
            raise NotFound("The user does not have an associated Employee profile.")
        
        # Add employee instance to the serializer data
        serializer = EmployeeLeaveRequestSerializer(data=request.data)
        
        if serializer.is_valid():
            # Save the serializer with the employee instance
